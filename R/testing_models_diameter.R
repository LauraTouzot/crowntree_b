diameter_models_nlme <- function () {
  ...
  data <- as.data.frame(cbind(df$DBH_cm, df$C_diam_m, df$location_ID, df$data))
  ...
  if (dim(data)[1] < 1000) { # running the models only if more than 1000 observations are left in the sampled data
    ...
    size <- floor(dim(data[data$x >= (class_dbh * 4 + 10),])[1] * 0.9)
    
    if (size >=  50) { 
      sample_size <- floor(dim(data[(data$x >= class_dbh * 3 + 10) & data$x < (class_dbh * 4 + 10),])[1] * 0.9) 
    } else {
    ...
    repeat {
      ...
      class_1 <- data[data$x >= 10 & data$x < (class_dbh + 10),] %>% sample_n(sample_size, replace = FALSE, prob = NULL)
      ...
      class_4 <- data[data$x >= (class_dbh * 3 + 10) & data$x < (class_dbh * 4 + 10),] %>% sample_n(sample_size, replace = FALSE, prob = NULL)
