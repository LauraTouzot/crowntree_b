diameter_models_nlme <- function (sp) {
  ...
  if (dim(data)[1] >= 50) { # running the models only if more than 1000 observations are left in the sampled data
    ...
    for (j in 1:nrep) {
      ...
      tryCatch({
        ...
        parameters_linear_2[j,paste0("protocol", levels(factor(new_data$protocol))[1])] <- fixed.effects(m2_ls)[1]
          ...
          parameters_linear_2[j,paste0("protocol", unique(new_data$protocol))] <- fixed.effects(m1_ls)[1]
